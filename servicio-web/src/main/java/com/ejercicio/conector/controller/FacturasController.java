...
import com.ejercicio.conector.dtos.FacturaDTO;import com.ejercicio.conector.exception.ParametrosInvalidosException;
    ...
    public ResponseEntity<List<FacturaDTO>> getFacturas(
        ...
        if(!fechaDesde.isEmpty() || !fechaHasta.isEmpty()){
            ...
            if(!fechaDesde.isEmpty() && !DateValidator.isValid(fechaDesde))
                throw new ParametrosInvalidosException("Formato de fechaDesde inválido, use YYYY-MM-DD");

            if(!fechaHasta.isEmpty() && !DateValidator.isValid(fechaHasta))
                throw new ParametrosInvalidosException("Formato de fechaHasta inválido, use YYYY-MM-DD");
        }

        List<FacturaDTO> response = service.getFacturas(cif, fechaDesde, fechaHasta);
